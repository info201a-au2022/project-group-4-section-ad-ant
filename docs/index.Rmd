...
------------- ------------ ------------
[Criminal Justice Data Book](https://sac.ofm.wa.gov/data)        Violence
Safety            Criminal-Activity
...
These charts represent the prison rate data from `r earliest_record` to `r latest_record` that’s compiled by WSAC (Washington Statistical Analysis Center). It measures the prison rate from `r number_of_counties` counties in Washington. The lowest prison rate is zero which is in `r lowest_prison_rate` and the highest prison rate is 36.3 in `r highest_prison_rate`. Meanwhile, the barchart could get the highest total crime and lowest total crime, which is `r highest_crime` and `r lowest_crime` respectively. From the data we are able to conclude that Snoqualmie is the safest city for citizens with a safety rating of `r safest_city`.
